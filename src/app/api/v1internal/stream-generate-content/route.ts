...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  // ── PAYLOAD CAPTURE (monitoring mode) ──────────────────────────────────────
  // Logs the full IDE request so we can identify the exact endpoint, headers,
  // and body structure that causes Google to register 5h quota consumption.
  if (pool === 'gemini') {
    ...
    console.log('[PAYLOAD CAPTURE] === GEMINI IDE REQUEST INTERCEPTED ===');
    console.log('[PAYLOAD CAPTURE] Headers:', JSON.stringify(capturedHeaders, null, 2));
    // Log the body (truncated at 2000 chars to avoid log flooding)
    const bodyPreview = bodyText.length > 2000 ? bodyText.slice(0, 2000) + '...[truncated]' : bodyText;
    console.log('[PAYLOAD CAPTURE] Body:', bodyPreview);
    console.log('[PAYLOAD CAPTURE] Body length:', bodyText.length, 'chars');
    console.log('[PAYLOAD CAPTURE] =============================================');
  }
  // ── END PAYLOAD CAPTURE ────────────────────────────────────────────────────
